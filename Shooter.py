...
sock = network.Sock()
destination = ["10.102.194.255", 33077] # adresse à laquelle on envoie les messages
...
# Définit une balle
class Bullet(Obj):
	def __init__(self, oid, pos, speed, color, damage):
		Obj.__init__(self, oid)
		self.pos = pos
		self.speed = speed
		self.color = color
		...
		for oid in objects: # pour tous les objets...
			...
			if obj.blocking(): # si l'objet est bloquant
				...
				if abs(self.pos[0]-obj.pos[0]) < 8 and abs(self.pos[1]-obj.pos[1]) < 8:
					obj.damage(self.damage) # causer des dommages
...
me = Player(name, [random.randint(16, w-16), random.randint(16, h-16)], random_color())
